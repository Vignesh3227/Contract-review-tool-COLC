...
with st.sidebar:
    ...
    if gemini_key:
        st.success("Google GenAI: Connected")
    else:
            ...
            for idx, issue in enumerate(issues):
                with st.container(border=True):
                    ...
                    selected_lang_name = st.selectbox(
                        "Voice Explainer Language", 
                        options=list(LANGUAGES.keys()), 
                        key=f"lang_sel_{idx}"
                    )
                    
                    target_lang_code = LANGUAGES[selected_lang_name]
                    cache_key_prob = f"{idx}_{target_lang_code}_problem"
                    cache_key_sol = f"{idx}_{target_lang_code}_solution"
                    
                    st.write("---")
                    st.caption(f"🔊 Listen in {selected_lang_name}:")
                    
                    audio_col1, audio_col2 = st.columns(2)
                    
                    with audio_col1:
                        if cache_key_prob in st.session_state['audio_cache']:
                            st.audio(st.session_state['audio_cache'][cache_key_prob], format="audio/wav")
                        else:
                            if st.button("Listen to Problem", key=f"btn_prob_{idx}", use_container_width=True):
                                with st.spinner("Generating..."):
                                    try:
                                        desc_raw = issue.get("description", "")
                                        desc_safe = desc_raw[:900] if len(desc_raw) > 900 else desc_raw
                                        translated_text = translate_text(desc_safe, target_lang=target_lang_code)
                                        audio_bytes = text_to_speech(translated_text, target_lang=target_lang_code)
                                        
                                        if audio_bytes:
                                            st.session_state['audio_cache'][cache_key_prob] = audio_bytes
                                            st.rerun()
                                    except Exception:
                                        st.error("Problem track failed.")

                    with audio_col2:
                        if cache_key_sol in st.session_state['audio_cache']:
                            st.audio(st.session_state['audio_cache'][cache_key_sol], format="audio/wav")
                        else:
                            if st.button("Listen to Revision", key=f"btn_sol_{idx}", use_container_width=True):
                                with st.spinner("Generating..."):
                                    try:
                                        redline_raw = issue.get("recommended_redline", "")
                                        redline_safe = redline_raw[:900] if len(redline_raw) > 900 else redline_raw
                                        
                                        prefix = "सुझाया गया संशोधन: " if target_lang_code == "hi-IN" else "ਸੁਝਾਇਆ ਗਿਆ ਸੁਧਾਰ: " if target_lang_code == "pa-IN" else "Suggested Revision: "
                                        
                                        translated_text = translate_text(redline_safe, target_lang=target_lang_code)
                                        full_native_text = f"{prefix}{translated_text}"
                                        audio_bytes = text_to_speech(full_native_text, target_lang=target_lang_code)
                                        
                                        if audio_bytes:
                                            st.session_state['audio_cache'][cache_key_sol] = audio_bytes
                                            st.rerun()
                                    except Exception:
                                        st.error("Revision track failed.")
